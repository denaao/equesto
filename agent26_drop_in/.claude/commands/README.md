...
| `/tasks/project-foundation` | Frame a new project — problem, MVP, roadmap, handoff bundle |
| `/tasks/mvp-structuring` | Cut an idea or oversized scope into the smallest viable MVP |
| `/tasks/context-discovery` | Fill specific context gaps mid-flight without full re-discovery |
...
> **Note on foundation work:** there is no `/squads/foundation-squad` by design. The Interactive Discovery & Project Foundation Agent operates as a sequential gate executed by a single agent — collaboration with Product Manager, UX Researcher, etc. happens via structured handoff after the foundation bundle is produced, not as a parallel squad review.
