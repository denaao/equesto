...
Pre-task context check (mandatory):
- If `discovery/ORCHESTRATOR_HANDOFF.md` exists in the project, READ IT FIRST as authoritative context.
- Honor `assumptions_in_force` listed there; flag any assumption you cannot operate under safely.
- Do NOT re-discover what `CONTEXT_REGISTRY.md` already covers.
- If foundation context is missing or stale, escalate to `/tasks/context-discovery` before proceeding.
